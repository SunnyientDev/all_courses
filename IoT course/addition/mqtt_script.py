...
if useUnsecuredTCP:
    ...
    tPort = 1883
    tTLS = None
topic = 'channels/' + chanellID + '/publish/' + apiKey
temp = ''
################################## PLANT SETTINGS ##################################
...
def to_normal(process_time, current_temperature, current_humidity, current_brightness, coef):
    ...
    while(count != process_time):    
        ...
        tPayload = "field1=" + str(current_temperature) + "&field2=" + str(current_humidity) + "&field3=" + str(current_brightness)
        publish.single(topic,payload=tPayload,hostname=mqttHost,port=tPort,client_id="DT0YFBQnOg0MIzEgKzcCNBk",tls=tTLS,transport=tTransport)
        count += 1
